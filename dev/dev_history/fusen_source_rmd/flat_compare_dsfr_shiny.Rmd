...
- Limiter par les paramètres modifiables.
...
```{r, echo=FALSE}
knitr::include_graphics(system.file("comparaison", "option_b_btn.png", package = "shinygouv"))
```
...
Ceci sera transformé en fonction et documenter dans un package
...
Les éléments tels que navbarpage, tabpanel etc sont possibles dans la limite de ce que propose DSFR. Ils necéssitent plus ou moins une demi-journée par composant (codé avec des htmlTemplates). 
...
fusen::inflate(flat_file = "dev/flat_compare_dsfr_shiny.Rmd", vignette_name = "Recommandation pour l'implementation de DSFR")
```
